    ...
    with open(file_path, "r", encoding="utf8") as f:
        try:
            ...
            if len(wine_description.find_all("p")) > 0:
                wine_description = "\n".join(
                    str(tag.text) for tag in wine_description.find_all("p")
                )
            else:
            ...
            wine_category_1 = soup.find("ul", attrs={"class": "prodAttr"}).find("li")[
                "title"
            ]
            ...
            # Extract size
            wine_size_value = soup.find(
                "span", attrs={"class": "prodAlcoholVolume_text"}
            ).text
            wine_size_units = re.findall(
                "[a-zA-Z]+",
                soup.find("span", attrs={"class": "prodAlcoholVolume"}).text,
            )[0]

            # Extract ABV
            wine_abv = soup.find(
                "span", attrs={"class": "prodAlcoholPercent_percent"}
            ).text
